...
| Scheduled retention | `retention.yml`, Mondays 11:29 UTC, runs the prune's proof and fails when the raw zone exceeds 1 GB. Reports only, never `--apply`. **Written 2026-08-09 and not yet run**: it fires on the first Monday after it reaches `main` | same | ADR-17 |
...
one tool here that deletes part of the record (ADR-14, PLAN-9).** Measured on
...
**The proof now runs weekly and the deletion still does not (ADR-17).** ADR-14
...
one (ADR-16).** ADR-10 cut `city_budget` and `street_trees` on 2026-08-04 and
...
2. `docs/decisions/` in number order. These are the constraints you inherit.
   ...
   - **ADR-4 still repays it in full.** ADR-9 supersedes it on two points,
     ...
     watermark all carry forward. **ADR-14 then amends the append-only rule
     with a second exception and ADR-16 with a third**, so those four read as
     one conversation about what the raw zone is, and ADR-14's own claim is
     that ADR-4's actual buy, `make rebuild` reproducing every model, is what
     had to survive it. ADR-17 is a fifth voice in the same conversation and
     the only one that deletes nothing: it moves the prune's *proof* onto a
     schedule and leaves the deletion where ADR-14 put it.
...
ingestion/          dataset_registry.py loads the dataset registry, which is
                    ...
                    (PLAN-9, ADR-14). Its `--max-bytes` mode is the half that
                    ...
                    prune has stopped being run (ADR-17). And
...
- **The Parquet raw zone is append-only, with exactly three exceptions.** Files
  ...
  upload the result. The second is `ingestion/prune_raw.py` (ADR-14), which
  ...
  deleting anything it cannot prove. The third is a **scope deletion** (ADR-16):
...
- **Snapshot datasets are prunable and delta datasets are never prunable, and
  ...
  one**, rejected in ADR-14 for exactly this reason: it deletes by object age
...
- Decisions: `docs/decisions/`. ADR-1 warehouse targets, ADR-2 spatial
  ...
  ADR-12 published export layout, ADR-13 context pack format, ADR-14 raw zone
  retention (amends ADR-4's append-only rule, and is itself amended by ADR-16
  and ADR-17), ADR-15 the bigquery pack is declared and not generated (amends
  the spec's section 2 table, not ADR-13), ADR-16 a dataset cut from the
  registry is deleted from the zone (the third exception to append-only),
  ADR-17 the prune's proof runs on a schedule and its deletion never does
  (amends ADR-14's answer to where the prune runs).
- Plans: `docs/plans/`
