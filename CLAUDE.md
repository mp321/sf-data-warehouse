...
| Context pack | the DuckDB one, generated by `make context-pack` and committed under `context-pack/`. `make context-pack-check` is the drift gate and is not in CI yet | three packs, one per target, and CI regenerating the two that need no credentials | PLAN-6, `docs/specs/context-pack.md` |
