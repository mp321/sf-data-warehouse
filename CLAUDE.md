...
| Raw zone | Parquet, read and written in whichever zone is configured: `data/raw`, or `gs://<bucket>/raw` when `RAW_ZONE_URI` is set. Append-only with two exceptions, `--full-refresh` and the prune | same | ADR-4, ADR-9, ADR-14 |
...
| Context pack | two of three: the DuckDB one and the published one, generated by hand with `make context-pack [TARGET=published]` and committed under `context-pack/`. CI runs the drift check for both on every PR, against the fixture warehouse and the fixture export, and generates nothing | three packs, one per target, each generated by hand and checked in CI | PLAN-6 closed, PLAN-8 steps 1 to 4 done, ADR-13, `docs/specs/context-pack.md` |
...
2. `docs/decisions/` in number order. These are the constraints you inherit.
   ...
   - **ADR-4 still repays it in full.** ADR-9 supersedes it on two points,
     ...
     with a second exception**, so those three read as one conversation about
     what the raw zone is, and ADR-14's own claim is that ADR-4's actual buy,
     `make rebuild` reproducing every model, is what had to survive it.
...
tools/context_pack/ the context pack generator (PLAN-6 step 2), verified
                    ...
                    open_target raises for it (PLAN-8 step 6). An entry's
...
- **The Parquet raw zone is append-only, with exactly two exceptions.** Files
  ...
  deleting anything it cannot prove. **Nothing edits a file in the zone**, which
  is the part of the rule that has no exceptions at all. Warehouse raw tables
...
- Decisions: `docs/decisions/`. ADR-1 warehouse targets, ADR-2 spatial
  ...
  retention (amends ADR-4's append-only rule).
- Plans: `docs/plans/`
