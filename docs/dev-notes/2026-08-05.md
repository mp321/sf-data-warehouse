...
Nothing to execute. Checked by reading against sources instead:

- Every number traces to a file. The r8 and r10 agreement table to ADR-6's
  measurements section, the coordinate drop rates to
  `_marts__models.yml`'s `coordinate_drop_rate_pct`, the cell areas and the
  mart resolution to `vars` in `dbt_project.yml`, the 874 months and the 1849 to
  2028 range to the `PUBLISHED_MARTS` comment, the population figures to ADR-6.
- Every model, column and ADR cited exists today, checked against
  `dbt/models/**/*.yml` rather than from memory. 19 models, which is what
  `find dbt/models -name '*.sql'` counts.
- `upstream_supervisor_district` was confirmed present on all four point staging
  models before the district refusal was written, because that refusal only
  matters if the column is there to be joined on by mistake.

One thing found and not fixed, since it is not this session's change:
`mart_activity_by_h3` has a `category` column in the SQL and in its
`unique_combination` test, and no entry for it in `_marts__models.yml`. The
generator will surface it as a column with no description.

### For the next session

PLAN-6 step 2, the generator, at `tools/context_pack/`. The spec is the
contract; read section 3 for the artifact shape, section 7 for the one YAML that
holds the hand-written prose, and sections 8 and 9 for the two failure rules
that are easy to implement as warnings by mistake. Both must fail the build.
...
- `make check`: green, exit 0. 143 pytest passed, up from 127, ruff and sqlfluff
  clean, leak-check clean, `make ci-build` `PASS=171 ERROR=0` on both passes.
  The new step reconciled 7 manifests on the CI fixture zone.
- The real local zone: 19 manifests across 9 dataset directories, 0 failing, 2
  warnings for the ADR-10 leftovers.
- **Broken three ways, on a zone built by the same fixture ingest `ci-build`
  runs.** A manifest edited to claim 999 rows: exit 3, naming
  `raw_311_cases run 20260806T051924Z`, 999 claimed against 26 held. A manifest
  deleted: exit 4, naming the run and its 26 rows in 1 file under
  `ingest_date=2026-08-06`. A Parquet file deleted: exit 3, "manifest claims 681
  row(s) in 1 file(s), the zone holds none". A watermark edited forward: warning
  only, exit 0. An empty zone: "no run manifests in the zone", exit 0.
- Note `make check-runs` reports make's exit 2 rather than the script's 3 or 4,
  which is make's behaviour and is true of `make check-derived` too. `ci.yml`
  calls the script directly, so the distinct codes survive in CI.
- Not run: `make build-bigquery`, `make parity-columns`, `make parity-check`.
  This session added a zone reader and touched no SQL, no column list, no
  registry entry and nothing in `load.py`.

### For the next session

1. **PLAN-6 is the only open plan**, and the next session is step 2, the
   generator at `tools/context_pack/`, against `docs/specs/context-pack.md`.
   The prompt is in `docs/handoff-prompt.md`, which is now one session long.
2. **The working tree is four sessions deep and unpushed.** PLAN-5 step 13,
   PLAN-6 step 1 and this session are all in it. Worth committing before
   starting the generator, so a PLAN-6 session's diff is its own.
3. Unchanged and still human-only: the two orphaned BigQuery external tables,
   the unobserved first cron run from `main`, and one credentialed
   `make spatial` to give the bucket's derived zone a code stamp.
