...
The lesson is the same one this session keeps producing, one level up. The
comment in `ci.yml` says "Compiling does not open a warehouse connection", and
it was written as an explanation rather than as an assertion, so nothing
noticed when it stopped being true. A comment claiming a property that a
dependency bump can revoke is a test that never runs.
