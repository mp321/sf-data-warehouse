...
0. Set the `GCP_SA_KEY` secret to the entire contents of `keys/sa.json`, delete
   the `SOCRATA_APP_TOKEN` secret or set it to a real token, then push all three
   corrections and re-dispatch. None of the three failed runs wrote anything:
   the first died at workflow validation, the second before the first object was
   read, the third before the first Socrata request was sent.
