...
| Target | Models | Freshness from | Examples verified against | Credentials | Generated by |
|---|---|---|---|---|---|
| `duckdb` | all 19 | last `make build` | the DuckDB file | none | CI, every PR |
| `published` | the 6 marts in `PUBLISHED_MARTS` | `published/manifest.json` | DuckDB over `published/*.parquet` | none | CI, every PR |
| `bigquery` | all 19 | last `make build-bigquery` | BigQuery | yes | by hand |
...
- **Two of the three regenerate with no credentials**, because DuckDB reads
  Parquet directly and CI already writes `published/` on every pull request
  (ADR-8). That is what keeps PLAN-6 step 4 honest: drift shows up as a diff in
  the pull request for the two targets a fork can build.
...
is a constraint that has to overturn something rather than shape it.
...
not fit the budget with all three present, generation fails rather than emitting
a truncated pack. A pack missing a refusal is worse than no pack, because it
reads complete.
